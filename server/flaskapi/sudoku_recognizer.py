...
def recognize_grid_from_array(image_array):
    preprocessed = preprocess(image_array)
    contour = find_biggest_contour(preprocessed, image_array)
    if contour is None:
        raise ValueError("Sudoku grid not found in image.")

    warped = warp_grid(image_array, contour)
    cells = extract_cells_from_grid(warped)

    sudoku_grid = np.zeros((9, 9), dtype=int)
    with concurrent.futures.ProcessPoolExecutor(initializer=init_process) as executor:
        flat_cells = [cell for row in cells for cell in row]
        results = executor.map(process_cell, flat_cells)
...
if __name__ == "__main__":
    try:
        sudoku_grid = process_sudoku("2.jpg")
